...
vows.describe("Scripts").addBatch(
  ...
  "with entities":
    topic: ->
      brains.get "/script/split", (req, res)-> res.send """
        <html>
  ...
  "with eval":
    ...
    "should evaluate in global scope": (browser)-> assert.equal browser.document.title, "OneTwoThree"
  ...
  "run without scripts":
    topic: ->
      brains.get "/script/no-scripts", (req, res)-> res.send """
        <html>
          <head>
            <title>Zero</title>
        ...
        """
      brains.get "/script/no-scripts.js", (req, res)-> res.send "document.title = document.title + 'One'"
