...
vows.describe("History").addBatch(
  "new window":
    ...
    "should start out empty": (window)-> assert.lengthOf window.history, 0
  ...
  "history":
    "pushState":
      zombie.wants "http://localhost:3003/"
        ...
        "should add state to history": (window)-> assert.lengthOf window.history, 3
    ...
    "replaceState":
      zombie.wants "http://localhost:3003/"
        topic: (browser)->
          ...
          @callback null, browser.window
        "should not add state to history": (window)-> assert.lengthOf window.history, 2
        "should change location URL": (window)-> assert.equal window.location.href, "/end"
        "go backwards":
          topic: (browser)->
  ...
  "location":
    "open page":
      zombie.wants "http://localhost:3003/"
        "should add page to history": (browser)-> assert.lengthOf browser.window.history, 1
    ...
    "change pathname":
      zombie.wants "http://localhost:3003/"
        topic: (browser)->
          ...
          return
        "should add page to history": (browser)-> assert.lengthOf browser.window.history, 2
    ...
    "replace":
      zombie.wants "http://localhost:3003/"
        topic: (browser)->
          ...
          browser.window.document.addEventListener "DOMContentLoaded", => @callback null, browser
          return
        "should not add page to history": (browser)-> assert.lengthOf browser.window.history, 1
        "should change location URL": (browser)-> assert.equal browser.location, "http://localhost:3003/history/boo"
        "should load document": (browser)-> assert.match browser.html(), /Eeek!/
        "should load document in new window": (browser)-> assert.ok browser.window != @window
    "reload":
  ...
  "redirect":
    zombie.wants "http://localhost:3003/history/redirect"
      ...
      "should not add location in history": (browser)-> assert.lengthOf browser.window.history, 1
  ...
  "redirect back":
    zombie.wants "http://localhost:3003/history/boo"
      ...
      "should not add location in history": (browser)-> assert.lengthOf browser.window.history, 2
      "should indicate last request followed a redirect": (browser)-> assert.ok browser.redirected
).export(module)
